...
import { BlazeConnectionService } from './../blaze-connection/blaze-connection.service';
...
export class BotService {
  ...
  attachEvents() {
    ...
    this.broker.on(
      'admin-bots-retrieved',
      this.onAdminBotsRetrieved.bind(this)
    );
    this.broker.on(
  ...
  startBot() {
    ...
    const { walletId, accessToken } = this.blazeConnectionService;

    let args: any[] = [this.accountType];

    this.accountType === 'REAL' &&
      (args = args.concat([walletId, accessToken]));
